...
export class GameAudio {
  ...
  init(camera) {
    ...
    const unlock = () => {
      ...
      this.canvas.removeEventListener("touchstart", unlock);
      // this is our controls in NativeScript;
      this.canvas.page
        .getViewById("touch-controls")
        .removeEventListener("touch", unlock, this);
    };

    window.addEventListener("keydown", unlock);
    window.addEventListener("click", unlock);
    this.canvas.addEventListener("touchstart", unlock);

    // this is our controls in NativeScript; we have to use the page to get the view and add the listener there
    this.canvas.page
      .getViewById("touch-controls")
      .addEventListener("touch", unlock, this);
  }
  ...
  update(dt, speed, throttle, driftIntensity) {
    ...
    const inGear = THREE.MathUtils.clamp(
      (absSpeed - gearStart) / gearWindow,
      0,
      1,
    );
    ...
    this.skidSound.setVolume(
      THREE.MathUtils.lerp(curSkidVol, skidVol, dt * 10),
    );

    const skidPitch = THREE.MathUtils.clamp(Math.abs(speed), 1, 3);
    const curSkidPitch = this.skidSound.getPlaybackRate();
    this.skidSound.setPlaybackRate(
      THREE.MathUtils.lerp(curSkidPitch, skidPitch, 0.1),
    );
  }
  ...
  playImpact(impactVelocity) {
    ...
    const volume = THREE.MathUtils.clamp(
      remap(impactVelocity, 0, 6, 0.01, 1.0),
      0.01,
      1.0,
    );
